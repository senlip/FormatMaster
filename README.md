...
```bat
python tests\selftest.py            :: 通用转换端到端，30 项
python tests\selftest_decrypt.py    :: 解密层逐字节校验，33 项（含官方金标）
python tests\selftest_pipeline.py   :: 调度链路：剥壳 → 转码，19 项
python tests\selftest_ui.py         :: 界面集成（无头运行），19 项
```

四套共 **101 项**，全部通过。测试素材自动生成，解密相关三套按各平台算法
